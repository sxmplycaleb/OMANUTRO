  ...
  function setupPhoneCopy() {
    document.querySelectorAll("[data-copy-phone]").forEach((link) => {
      link.addEventListener("click", async (event) => {
        ...
        const status = link.parentElement?.querySelector(".copy-confirmation");
        const shouldOpenDialer = window.matchMedia("(hover: none), (pointer: coarse), (max-width: 760px)").matches;

        if (!shouldOpenDialer) event.preventDefault();

        try {
          await navigator.clipboard.writeText(phone);
          if (status) status.textContent = "Copied";
          showAlert(shouldOpenDialer ? "Opening dialer. Phone number copied." : "Phone number copied.", "success");
        } catch (error) {
          if (!shouldOpenDialer) {
        ...
        clearTimeout(link.copyTimeout);
        link.copyTimeout = setTimeout(() => {
