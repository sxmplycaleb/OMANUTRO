...
function renderProducts() {
  ...
  if (resultCount) resultCount.textContent = `${state.products.length} item${state.products.length === 1 ? "" : "s"}`;
...
function renderHeroPreview() {
  ...
  if (!state.products.length) {
    ...
    if (featureImage) {
      featureImage.src = "/assets/brands/dcash-cutout.png";
...
function setButtonLabel(button, label) {
  if (!button) return;
  const labelNode = button.querySelector("span:last-child") || button.querySelector("span");
...
function normalizedPhoneValue() {
  ...
  const code = $("#authCountryCode")?.value || "+254";
...
function validatePasswordPolicy(password, strict = false) {
  const score = passwordScore(password);
  if (strict && score < 3) throw new Error("Use a stronger password with uppercase, lowercase, numbers, or symbols.");
  return score;
}

function setFieldFeedback(id, message, ok = false) {
  const node = document.querySelector(`[data-feedback-for="${id}"]`);
  if (!node) return;
  node.textContent = message || "";
  node.classList.toggle("is-valid", Boolean(message && ok));
...
function syncAuthValidation() {
  ...
  if (password) {
    ...
    if (strength) {
      ...
      strength.querySelector("span").textContent = score >= 4 ? "Strong password" : score >= 3 ? "Good password" : "Add uppercase, lowercase, number, or symbol";
    }
    setFieldFeedback("authPassword", score >= 3 ? "Password looks good" : "Use at least 8 characters with variety.", score >= 3);
  }
  if (confirm) setFieldFeedback("authPasswordConfirm", confirm === password ? "Passwords match" : "Passwords do not match.", confirm === password);
  if (resetIdentifier) setFieldFeedback("resetIdentifier", isValidEmail(resetIdentifier) || isValidPhone(resetIdentifier) ? "Valid account identifier" : "Enter a valid email or phone.", isValidEmail(resetIdentifier) || isValidPhone(resetIdentifier));
}
...
function validateAuthForm(body) {
  ...
  if (state.signupStep === "code") {
    if (!$("#authSignupCode")?.value || $("#authSignupCode").value.length < 6) throw new Error("Enter the six-digit verification code.");
    return;
  }
...
function hydrateOtpInputs() {
  $$(".otp-grid").forEach((grid) => {
    const source = document.getElementById(grid.dataset.otpTarget);
    if (!source || grid.children.length) return;
    source.classList.add("visually-hidden-input");
    for (let index = 0; index < 6; index += 1) {
      ...
      input.ariaLabel = `Digit ${index + 1}`;
      input.addEventListener("input", () => {
        input.value = input.value.replace(/\D/g, "").slice(0, 1);
        source.value = [...grid.querySelectorAll(".otp-input")].map((node) => node.value).join("");
      ...
      input.addEventListener("paste", (event) => {
        ...
        grid.querySelectorAll(".otp-input").forEach((node, digitIndex) => {
          node.value = digits[digitIndex] || "";
        });
        source.value = digits;
        grid.querySelectorAll(".otp-input")[Math.min(digits.length, 5)]?.focus();
      });
      grid.append(input);
    }
  });
...
function updateResetPasswordUI() {
  ...
  if (state.resetStep === "email") {
    if (submit) setButtonLabel(submit, "Send code");
    if (help) help.textContent = "Enter your account email or phone and we will send a reset code.";
  } else if (state.resetStep === "code") {
    if (submit) setButtonLabel(submit, "Verify code");
    if (help) help.textContent = "Enter the six-digit reset code.";
  } else {
    if (submit) setButtonLabel(submit, "Update password");
...
async function submitResetPassword(event) {
  ...
  try {
    ...
    validatePasswordPolicy($("#resetPassword")?.value || "", true);
...
function showDcashPopup() {
  ...
  if (dcashPopup) {
    ...
    $("#closeDcashPopup")?.focus({ preventScroll: true });
  }
}
...
function bindEvents() {
  ...
  if (dcashPopup && closeDcashPopup) {
    ...
    dcashPopup.addEventListener("keydown", (event) => {
      if (event.key === "Escape") hideDcashPopup();
    });
  }
  ...
  if (mobileMenuButton && mobileMenuButton.dataset.mobileNavBound !== "true") {
    mobileMenuButton.dataset.mobileNavBound = "true";
  ...
  $$(".nav-tab").forEach((tab) => tab.addEventListener("click", () => {
    if (tab.classList.contains("nav-more-trigger")) return;
    if (!tab.dataset.view) {
  ...
  $$("#authForm input, #resetForm input").forEach((input) => {
    input.addEventListener("input", syncAuthValidation);
  ...
  $("#authCountryCode")?.addEventListener("change", syncAuthValidation);
